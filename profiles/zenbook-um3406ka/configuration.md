...
|---|---|---|---|---|
| Monitor | [monitors.lua](monitors.lua) | `scripts/bootstrap.sh --profile zenbook-um3406ka --no-vpn --no-terminal` | [display tests](docs/display-tests.md) and `hyprctl monitors all` | Latest `~/.local/state/omarchy-profiles/zenbook-um3406ka/backups/*/monitors.lua` |
| Packages | `packages/platform.txt`, `packages/diagnostics.txt`, `packages/terminal.txt` | Bootstrap or `omarchy-pkg-add` | [diagnostics](docs/diagnostics.md), `pacman -Q` | Packages are additive; remove only after a dependency review |
