...
| Packages | Native Omarchy package set plus voice-scoped [voice-npu.txt](../../profiles/zenbook-um3406ka/packages/voice-npu.txt); optional [diagnostics.txt](../../profiles/zenbook-um3406ka/packages/diagnostics.txt) | Bootstrap installs the NPU runtime only when voice is enabled; diagnostics remain explicit | [diagnostics](../evidence/diagnostics.md), `pacman -Q` | Packages are additive; remove only after a dependency review |
