...
# Canonical one-command entry point for a clean Omarchy restore.
# Keep the compatibility install.sh wrapper for existing bookmarks.
...
if ((check_only || plan_only)); then
  ...
  bash -n "$check_dir/repo/sh.sh" "$check_dir/repo/scripts/bootstrap.sh"
  if ((plan_only)); then
